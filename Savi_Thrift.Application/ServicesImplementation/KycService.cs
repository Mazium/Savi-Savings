...
namespace Savi_Thrift.Application.ServicesImplementation
{
    public class KycService : IKycService
	{
		...
		public async Task<ApiResponse<KycResponseDto>> GetKycByUserId(string userId)
		{
			var kycs = await _unitOfWork.KycRepository.FindKycs(k => k.AppUserId == userId);
			if (kycs.Any())
			{
				var kyc = kycs.First();
				var response = _mapper.Map<KycResponseDto>(kyc);
				return ApiResponse<KycResponseDto>.Success(response,"KYC retrieved successfully", StatusCodes.Status200OK);
			}
			else
			{
				return ApiResponse<KycResponseDto>.Failed("KYC details not found",StatusCodes.Status404NotFound, new List<string>());
			}
		}
